...
int main(int argc, char** argv)
{
   ...
   int tam_trozos = rows/(num_procesos-1); // +1 para transición de un procesador a
   ...
   if (id_proceso == 0) {  // Procesador de reparto/recepción
      ...
      for (int i=1; i<num_procesos; i++) {
         int ini = (i-1)*tam_trozos,  
             fin = i*tam_trozos + (i!=num_procesos-1 ? 1 : 0);
          // ini: Iniciamos una fila atrás, para respetar vecindad
          // fin: Si es último rango, no suma 1 -> fin imagen

         Mat trozo ( src, (num_procesos==2 ? Range::all() : Range(ini, fin)) );

         MPI_Send(trozo.data, (trozo.cols*trozo.rows), MPI_UNSIGNED_CHAR, i, 0, MPI_COMM_WORLD);
      }

      Mat dst;
      dst.create((num_procesos==2 ? 0 : tam_trozos), src.cols, src.type());


      double ini_recepcion, t_ejec;
      for (int i=1; i<num_procesos; i++) {
